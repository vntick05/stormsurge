    ...
    for line in lines:
        ...
        if skipping:
            ...
            if APPENDIX_HEADING_RE.match(line):
                continue
            if "Removed for Classification" in line:
                continue
            if not line:
                continue
        else:
...
def heading_to_markdown(line: str) -> str | None:
    ...
    if not match:
        plain = PLAIN_SECTION_HEADING_RE.match(line)
        if plain:
            match = plain
        else:
            return None
    ...
    if NON_HEADING_PREFIX_RE.match(title):
        return None
    if title.lower().startswith("table is unclassified"):
        return None
...
def transform_lines(lines: list[str]) -> list[str]:
    ...
    for line in lines:
        ...
        if "Removed for Classification" in line:
            continue
        if line.lower() == "table is unclassified":
            continue
